...
class Image_loader(object):
    ...
    def load_and_process_image(self):
        ...
        if do_or:
            ...
            im4d = im.unsqueeze(0).unsqueeze(0)
            # Calculate median using a square kernel approximation of the disk
            kernel_size = 2 * radius + 1
            median_filtered = kornia.filters.median_blur(im4d, (kernel_size, kernel_size))
            # Find outliers where the original image deviates too much from the median
            outlier_mask = (im4d > median_filtered + threshold) | (im4d < median_filtered - threshold)
            # replace outliers with median value             
            output_im = torch.where(outlier_mask, median_filtered, im4d)

            im = output_im.squeeze()
            im.to(self.device)
        
        default_image = im.cpu().numpy()
